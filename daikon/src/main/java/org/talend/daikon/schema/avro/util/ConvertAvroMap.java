...
public class ConvertAvroMap<DatumT, AvroT> implements HasNestedAvroConverter<Map<String, DatumT>, Map<String, AvroT>> {
    ...
    public Map<String, DatumT> convertToDatum(Map<String, AvroT> value) {
        return Collections.unmodifiableMap(
                new MappedValueMap<>(value, new ConvertAvroList.LambdaConvertToDatumFunction<AvroT, DatumT>(elementConverter),
                        new ConvertAvroList.LambdaConvertToAvroFunction<DatumT, AvroT>(elementConverter)));
    }

    @Override
    public Map<String, AvroT> convertToAvro(Map<String, DatumT> value) {
        return Collections.unmodifiableMap(
                new MappedValueMap<>(value, new ConvertAvroList.LambdaConvertToAvroFunction<DatumT, AvroT>(elementConverter),
                        new ConvertAvroList.LambdaConvertToDatumFunction<AvroT, DatumT>(elementConverter)));
    }
